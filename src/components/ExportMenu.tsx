...
import { exportToPdf, exportToTxt, exportToDocx } from '../lib/exporter';
...
export function ExportMenu({ song, options, printRef }: Props) {
  ...
  const handle = async (type: 'pdf' | 'txt' | 'docx') => {
    setLoading(type);
    setOpen(false);
    try {
      if (type === 'pdf') {
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-2 z-20 bg-white border border-stone-200 rounded-xl shadow-lg overflow-hidden min-w-[160px]">
            <button
              onClick={() => handle('pdf')}
